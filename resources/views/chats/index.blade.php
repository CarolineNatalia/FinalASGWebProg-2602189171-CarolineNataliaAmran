...
@section('content')
    <div class="container py-3" style="overflow-y: scroll; height: 80vh;">
        <h1>{{ __('chats_index.friend_list') }}</h1>
        <a href="{{ route('friends.friend_requests', ['locale' => app()->getLocale()]) }}"
            class="btn btn-secondary">{{ __('chats_index.add_friend_request') }}</a>
        @if ($friends->isEmpty())
            <p>{{ __('chats_index.no_friends') }}</p>
        @else
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            @if (!$item->visibility)
                                <div class="ms-3">
                                    ...
                                    <span class="badge bg-danger">{{ __('chats_index.hidden') }}</span>
                                </div>
                            @else
                                <div class="ms-3">
                                    <h5 class="card-title">{{ $item->name }}</h5>
